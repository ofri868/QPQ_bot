...
with open('items.txt', 'x', encoding='utf-8') as f:
    ...
    for index, row in df.iterrows():
        if re.match(r'^Weapon|^Gear', row['Config']):
            ...
            if re.match(r'.tail.|.extension cord.', row['Config'], re.IGNORECASE):
                f.write("{"+f"\"{row['Name']}\", \"Armor Rear\""+"},")
            elif re.match(r'.ankle.|.slippers.|.trotters.', row['Config'], re.IGNORECASE):
...
def parse_csv():
    with open('items.txt', 'r', encoding='utf-8') as f:
        ...
        for index, row in df.iterrows():
            if re.match(r'^Weapon|^Gear', row['Config']):
                items.append({'name': row['Name'], 'type': 'Gear'})
            elif re.match(r'^Costume', row['Config']):
                items.append({'name': row['Name'], 'type': 'Costumes'})
            elif re.match(r'^Accessory/Armor/Aura', row['Config']):
                items.append({'name': row['Name'], 'type': 'Armor Aura'})
            elif re.match(r'^Accessory/Armor/Back', row['Config']):
                if re.match(r'.tail.|.extension cord.', row['Config'], re.IGNORECASE):
                    items.append({'name': row['Name'], 'type': 'Armor Rear'})
                elif re.match(r'.ankle.|.slippers.|.trotters.', row['Config'], re.IGNORECASE):
                    items.append({'name': row['Name'], 'type': 'Armor Ankle'})
                items.append({'name': row['Name'], 'type': 'Armor Back'})
            elif re.match(r'^Accessory/Armor/Front', row['Config']):
                items.append({'name': row['Name'], 'type': 'Armor Front Arms'})
            elif re.match(r'^Accessory/Helm/Back', row['Config']):
                items.append({'name': row['Name'], 'type': 'Helm Back'})
            elif re.match(r'^Accessory/Helm/Front', row['Config']):
                items.append({'name': row['Name'], 'type': 'Helm Front'})
            elif re.match(r'^Accessory/Helm/Side', row['Config']):
                items.append({'name': row['Name'], 'type': 'Helm Side'})
            elif re.match(r'^Accessory/Helm/Top', row['Config']):
                items.append({'name': row['Name'], 'type': 'Helm Top Brow'})
            else:
                items.append({'name': row['Name'], 'type': 'Misc'})
        return items